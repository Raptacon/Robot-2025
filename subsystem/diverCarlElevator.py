...
class DiverCarlElevator(commands2.Subsystem):
    ...
    def periodic(self) -> None:
        # update telemtry
        if self.atGoal():
            self._trackingAlert.setText(f"Elevator stable at {self._encoder.getPosition():1.1f}cm")
            self._trackingAlert.set(True)
        else:
            self._trackingAlert.setText(f"Elevator at {self._encoder.getPosition():1.1f}cm goal {self.getSetHeightCm():1.1f}cm")
            self._trackingAlert.set(True)
        ...
        if self.getForwardLimit():
            self._limitAlert.setText(f"Elevator TOP HIT at {self._encoder.getPosition()}cm")
            self._limitAlert.set(True)
            return

        if self.getReverseLimit() and self._curentGoal > 0:
            self._primaryMotor.set(0)
            self._limitAlert.setText(f"Elevator BOTTOM HIT at {self._encoder.getPosition()}cm")
            self._limitAlert.set(True)
            return
